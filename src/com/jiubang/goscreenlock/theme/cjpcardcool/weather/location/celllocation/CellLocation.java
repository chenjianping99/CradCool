...
import com.jiubang.goscreenlock.theme.cjpcardcool.weather.location.SuperLocation;
...
public class CellLocation extends SuperLocation implements LocationListener
{
	...
	public boolean startLocation(int prevLocationWay, final ReqLocationListener reqLocationListener)
	{
		...
		// 2013.6.28:lishen-去掉sim卡状态的判断
		/*if (Util.getSimState(mContext) != TelephonyManager.SIM_STATE_READY) {
		...
		else
		{
			...
			if (Util.isInternalUser(mContext))
			{
				startMapbarLocate();
			}
			else
